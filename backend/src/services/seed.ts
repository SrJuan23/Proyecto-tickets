...
export async function seedDatabase(): Promise<void> {
  ...
  const agentePass = await bcrypt.hash('agente123', 10);
  ...
  for (const t of ticketData) {
    const res = await db.run(
      `INSERT INTO tickets (prioridad, cliente_id, asunto, descripcion, plataforma_id, solicitante, fecha_creacion, servicenow, turno, agente_id, estado, fecha_actualizacion, fecha_cierre, tiempo_atencion_minutos)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
