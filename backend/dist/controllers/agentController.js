...
async function createAgent(req, res) {
    try {
        ...
        const exists = await db_1.db.get('SELECT id FROM usuarios WHERE LOWER(nombre) = LOWER(?)', [nombre.trim()]);
        if (exists) {
        ...
        const result = await db_1.db.run(`INSERT INTO usuarios (nombre, email, telefono, especialidad, estado, rol, password_hash) VALUES (?, ?, ?, ?, ?, 'AGENTE', ?)`, [
            nombre.trim(),
...
async function updateAgent(req, res) {
    try {
        ...
        if (especialidad !== undefined) {
            ...
            values.push(especialidad ? especialidad.trim() : null);
        }
        if (estado !== undefined) {
            updates.push('estado = ?');
            values.push(estado);
        ...
        values.push(id);
        await db_1.db.run(`UPDATE usuarios SET ${updates.join(', ')} WHERE id = ?`, values);
        res.json({
            success: true,
...
async function toggleAgentStatus(req, res) {
    try {
        const { id } = req.params;
        const existing = await db_1.db.get('SELECT estado FROM usuarios WHERE id = ? AND rol = ?', [id, 'AGENTE']);
        if (!existing) {
            res.status(404).json({ success: false, message: 'Agente no encontrado.' });
            return;
        }
...
async function deleteAgent(req, res) {
    try {
        const { id } = req.params;
        const existing = await db_1.db.get('SELECT id FROM usuarios WHERE id = ? AND rol = ?', [id, 'AGENTE']);
        if (!existing) {
            res.status(404).json({ success: false, message: 'Agente no encontrado.' });
            return;
        }
        await db_1.db.run('UPDATE tickets SET agente_id = NULL WHERE agente_id = ?', [id]);
