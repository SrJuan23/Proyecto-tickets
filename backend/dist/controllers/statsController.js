...
async function getKPIs(req, res) {
    try {
      ...
      FROM tickets
    `;
        const row = await db_1.db.get(sql);
        ...
        logger_1.logger.error('Error al calcular indicadores', { error: error.message, stack: error.stack });
...
async function getCharts(req, res) {
    ...
    `;
        ...
        logger_1.logger.error('Error al generar gráficas', { error: error.message, stack: error.stack });
